...
CREATE FUNCTION GetOrderDetails(@InputOrderID int) RETURNS TABLE
    AS
        ...
        SELECT
            O.OrderID,
            O.ClientID,
            ISNULL(O.TakeawayID, 'Order not for takeaway') as 'TakeAwayID',
            ISNULL(O.ReservationID, 'Order is not for reservation') as 'ReservationID',
            ISNULL(O.InvoiceID, 'Order does not have invoice.') as 'InvoiceID',
            PM.PaymentName,
            PS.PaymentStatusName,
            CONCAT(S.LastName, ' ', S.FirstName) as 'Employee',
            O.OrderSum,
            O.OrderDate,
            ISNULL(O.OrderCompletionDate, 'Order is pending') as 'OrderCompletionDate',
            O.OrderStatus,
            P.Name,
