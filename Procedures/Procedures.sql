...
VALUES
    (@CategoryName, @Description) END TRY BEGIN CATCH DECLARE @msg nvarchar(2048) = N'Blad dodania kategorii: ' + ERROR_MESSAGE();THROW 52000,
    @msg,
    1 END CATCH END
GO
-- Categoryadd
...
SET
    ...
    1 END IF @Size < 2 BEGIN;THROW 52000,
    N'Stolik musi mieć przynajmniej 2 miejsca.',
    1 END IF @Size IS NOT NULL BEGIN
UPDATE
    TABLES
SET
    ChairAmount = @Size
WHERE
    TableID = @TableID END END TRY BEGIN CATCH DECLARE @msg nvarchar(2048) = N'Bład edytowania stolika: ' + ERROR_MESSAGE();THROW 52000,
    @msg,
    1 END CATCH END
GO
--Modify table size 
...
SET
    ...
    N 'Nie ma takiego stolika.',
    1 END DECLARE @TableStatus bit
SELECT
    @TableStatus = isActive
FROM
    TABLES
WHERE
    TableID = @TableID IF @TableStatus = @Status BEGIN;THROW 52000,
    N'Stolik ma już taki status!.',
    1 END IF @Status IS NOT NULL BEGIN
UPDATE
    TABLES
SET
    isActive = @Status
...
VALUES
    (@TableID, @Size, @Status) END TRY BEGIN CATCH DECLARE @msg nvarchar(2048) = N'Bład edytowania stolika: ' + ERROR_MESSAGE();THROW 52000,
    @msg,
    1 END CATCH END
GO
--Addtable 
...
DELETE FROM
    TABLES
WHERE
    TableID = @TableID END TRY BEGIN CATCH DECLARE @msg nvarchar(2048) = N'Bład edytowania stolika: ' + ERROR_MESSAGE();THROW 52000,
    @msg,
    1 END CATCH END
GO
--Remove table 
...
BEGIN 
    BEGIN TRY 
    DECLARE @CityID int IF NOT EXISTS(
        SELECT
            *
        FROM
            Cities
    ...
    END
        ...
        VALUES
            (@CityID, @Street, @LocalNr, @PostalCode) END TRY 
            BEGIN CATCH 
            DECLARE @msg nvarchar(2048) = N'Bład dodania adresu: ' + ERROR_MESSAGE();THROW 52000, @msg, 1 
    END CATCH 
    END
GO
--Add address 
...
SET
    ...
    1 END IF @CompanyName IS NOT NULL
    ...
    1 END IF (@ClientType = 'C') BEGIN IF(@NIP IS NULL) BEGIN THROW 52000,
    N'Nip musi być określony dla klienta firmowego!',
    1 END IF(@CompanyName IS NULL) BEGIN THROW 52000,
    N'CompanyName musi być określony dla klienta firmowego!',
    1 END END IF (@ClientType = 'I') BEGIN IF(@LastName IS NULL) BEGIN THROW 52000,
    N'Nazwisko musi być określony dla klienta indywidualnego!',
    1 END IF(@FirstName IS NULL) BEGIN THROW 52000,
    N'Imie musi być określony dla klienta indywidualnego!',
    1 END END DECLARE @AddressID int;IF NOT EXISTS(
        SELECT
            *
        FROM
            Address
...
VALUES
    (@ClientID, @CompanyName, @NIP, @KRS, @REGON) END IF (@ClientType = 'I') BEGIN EXEC addPerson @FirstName,
...
VALUES
    (@ClientID, @PersonID) END END TRY BEGIN CATCH DECLARE @msg nvarchar(2048) = N'Bład dodania klienta: ' + ERROR_MESSAGE();THROW 52000,
    @msg,
    1 END CATCH END
GO
--addclient 
...
SET
    ...
    1 END DECLARE @ProductID int DECLARE @StartDate datetime DECLARE @EndDate datetime
SELECT
    @ProductID = ProductID
FROM
    Products
...
VALUES
    (
        ...
        @Price,
        @ProductID
    ) END TRY BEGIN CATCH DECLARE @msg nvarchar(2048) = N'Błąd dodania potrawy do menu: ' + ERROR_MESSAGE();THROW 52000,
    @msg,
    1 END CATCH END
GO
    --add Product to Menu 
...
WHERE
    MenuID = @MenuID
    AND ProductID = @ProductID END TRY BEGIN CATCH DECLARE @msg nvarchar(2048) = N'Błąd usunięcia potrawy z menu: ' + ERROR_MESSAGE();THROW 52000,
    @msg,
    1 END CATCH END
GO
-- remove Product From Menu 
...
VALUES
    (
        ...
        @Price,
        @ProductID
    ) END TRY BEGIN CATCH DECLARE @msg nvarchar(2048) = N'Błąd dodania menu: ' + ERROR_MESSAGE();THROW 52000,
    @msg,
    1 END CATCH END
GO
    ...
    1 END declare @invoiceNum nvarchar(50) = concat(
        'FV/',
...
SELECT
    @InvoiceID = ID
FROM
    @InvoiceIDs RETURN @InvoiceID END TRY BEGIN catch DECLARE @msg nvarchar(2048) = N'Błąd dodania faktury: ' + ERROR_MESSAGE();THROW 52000,
    @msg,
    1 END catch END
GO
...
VALUES
    (@PaymentStatusName) END TRY BEGIN CATCH DECLARE @msg nvarchar(2048) = N'Błąd dodania metody płatności do zamówienia: ' + ERROR_MESSAGE();THROW 52000,
    @msg,
    1 END catch END
GO
GO
...
VALUES
    (@PaymentMethodName) END TRY BEGIN CATCH DECLARE @msg nvarchar(2048) = N'Błąd dodania metody płatności do zamówienia: ' + ERROR_MESSAGE();THROW 52000,
    @msg,
    1 END catch END
GO
    ...
    1 END DECLARE @PaymentMethodID int;
SELECT
    @PaymentMethodID = PaymentMethodID
FROM
    PaymentMethods
...
SET
    PaymentMethodID = @PaymentMethodID
WHERE
    OrderID = @OrderID END try BEGIN catch DECLARE @msg nvarchar(2048) = N'Błąd zmiany metody: ' + ERROR_MESSAGE();THROW 52000,
    @msg,
    1 END catch END 
    
CREATE PROCEDURE [change payment method for invoice] @PaymentMethodName varchar(50),
...
CREATE PROCEDURE [change payment status for invoice] @PaymentStatusName varchar(50),
    ...
    1 END DECLARE @PaymentStatusID int;
SELECT
    @PaymentStatusID = PaymentStatusID
FROM
    PaymentStatus
...
SET
    PaymentStatusID = @PaymentStatusID
WHERE
    InvoiceID = @InvoiceID END try BEGIN catch DECLARE @msg nvarchar(2048) = N'Błąd zmiany statusu: ' + ERROR_MESSAGE();THROW 52000,
    @msg,
    1 END catch END 
    
CREATE PROCEDURE [change payment status for order] @PaymentStatusName varchar(50),
    @OrderID int AS BEGIN BEGIN TRY IF NOT EXISTS(
        SELECT
            OrderID
        FROM
...
SET
    PaymentStatusID = @PaymentStatusID
WHERE
    OrderID = @OrderID END try BEGIN catch DECLARE @msg nvarchar(2048) = N'Błąd zmiany statusu: ' + ERROR_MESSAGE();THROW 52000,
    @msg,
    1 END catch END 
    
CREATE PROCEDURE OrderInsertInstPay @ClientID int,
    ...
    1 END Declare @OrderIDTable TABLE (Id int) Declare @OrderID int DECLARE @PaymentMethodID int DECLARE @PaymentStatusID int
SELECT
    @PaymentStatusID = PaymentStatusID
FROM
    PaymentStatus
...
SET
    InvoiceID = @InvoiceID
WHERE
    OrderID = @OrderID RETURN @OrderID END try BEGIN catch DECLARE @msg nvarchar(2048) = N'Błąd dodania zamowienia: ' + ERROR_MESSAGE();THROW 52000,
    @msg,
    1 END catch END;
GO
...
WHERE
    ...
    @PaymentMethodName = @PaymentMethodName_,
    @PaymentStatusName = @PaymentStatusName_,
    @InvoiceID = @InvoiceID output END
UPDATE
    [Orders]
SET
    InvoiceID = @InvoiceID
...
SET
    STATUS = @Status
WHERE
    Reservation.ReseravtionID = @ReservationID END END TRY BEGIN CATCH DECLARE @msg nvarchar(2048) = N'Błąd edytowania rezerwacji: ' + ERROR_MESSAGE();THROW 52000,
    @msg,
...
SET
    ...
    N 'Nie mozna zamowic tego produktu, gdyz nie ma go obecnie w menu',
    1 END DECLARE @OrderDate DATE
SELECT
    @OrderDate = OrderDate
FROM
    Orders
WHERE
    OrderID = @OrderID DECLARE @CategoryName nvarchar(50)
SELECT
    @CategoryName = CategoryName
FROM
    Products
...
WHERE
    Products.Name = @ProductName IF DATEPART(WEEKDAY, @OrderDate) != 4
    AND DATEPART(WEEKDAY, @OrderDate) != 5
    AND DATEPART(WEEKDAY, @OrderDate) != 6
    AND @CategoryName LIKE 'sea food' BEGIN;THROW 52000,
...
FROM
    Products
WHERE
    Name = @ProductName declare @BasePrice money
SELECT
    @BasePrice = Price
FROM
    Products
    INNER JOIN Menu M2 ON Products.ProductID = M2.ProductID
WHERE
    startDate >= GETDATE()
    AND (
        endDate IS NULL
    ...
    AND M2.ProductID = @ProductID declare @CurrentValue money
SELECT
    @CurrentValue = OrderSum
FROM
    [Orders]
WHERE
    OrderID = @OrderId declare @ClientID int
SELECT
    @ClientID = ClientID,
    @OrderDate = OrderDate
FROM
    Orders
WHERE
    OrderID = @OrderId declare @DiscMulti decimal(3, 2);
SELECT
    @DiscMulti = dbo.calculateDiscountForOrder(@ClientID)
INSERT INTO
...
UPDATE
    Orders
SET
    OrderSum = @CurrentValue + (@BasePrice * @Quantity * @DiscMulti)
WHERE
    OrderID = @OrderID END TRY BEGIN CATCH DECLARE @msg nvarchar(2048) = 'Błąd dodania produktu do zamowienia: ' + ERROR_MESSAGE();THROW 52000,
    @msg,
    1 END CATCH END
GO
...
GROUP BY
    ...
    P.Name END try BEGIN catch DECLARE @msg varchar(2048) = N'Bład wyświetlenia danych: ' + ERROR_MESSAGE();THROW 52000,
    @msg,
    1 END catch END
GO
...
WHERE
    ClientID = @ClientID
    AND PaymentStatusID = @PaymentStatusID END try BEGIN catch DECLARE @msg nvarchar(2048) = 'Błąd wyswietlenia statystyk o kliencie: ' + ERROR_MESSAGE();THROW 52000,
    @msg,
    1 END catch END
GO